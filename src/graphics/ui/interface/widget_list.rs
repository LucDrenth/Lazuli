...
pub struct WidgetEntry<T: UiWidget, U: Clone> {
    pub widget: T,
    pub id: u32,
    pub update_result: U,
}

impl <T: UiWidget, U: Clone> WidgetEntry<T, U> {
...
pub struct WidgetList<T: UiWidget, U: Clone> {
    pub entries: Vec<WidgetEntry<T, U>>,
}

impl <T: UiWidget, U: Clone> WidgetList<T, U> {
    pub fn new() -> Self {
        Self { entries: vec![] }
    }
    ...
    pub fn get_update_result(&self, widget_id: u32, default: U) -> U {
        for entry in self.entries.iter() {
            if entry.id == widget_id {
        ...
        log::engine_warn(format!("WidgetList get_update_result returns default because widget with id {} was not found", widget_id));
        default
    }
